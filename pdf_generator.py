...
def generate_proposal(proposal_type, client_name, replacements):
    """Generate proposal document based on proposal type"""
    # Map proposal types to their exact template filenames
    template_mapping = {
        "Digital Marketing": "DM proposal.docx",
        "AI Automation": "Ai_automation.docx",
        "AI Automation without LPW": "AI Automations Proposal wiithout lpw.docx",
    ...
    template_name = template_mapping.get(proposal_type)
    if not template_name:
        st.error(f"Unknown proposal type: {proposal_type}")
        return None
